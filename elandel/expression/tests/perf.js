import { test, is } from 'testra'

const src = (c) =>
  `1 + (a * b / c % d) - 2.0 + -3e-3 * +4.4e4 / f.g[0] - i.j(+k == 1)(${c})`
const args = {
  a: 123,
  b: 234,
  c: 345,
...
test.skip('expr-eval', async (t) => {
  ...
  let eeparser = new Parser({ logical: true }),
    expr = eeparser.parse(src(0))
  // console.log(expr);
...
test.skip('jexpr', async (t) => {
  ...
  const { parse, EvalAstFactory } = await import(
    'https://cdn.skypack.dev/jexpr'
  )
...
test.skip('jexl', async (t) => {
  ...
  let jexl = new Jexl(),
    expr,
    jexlArgs = Object.assign({}, args, { i: [1] }),
...
test.skip('math-expression-evaluator', async (t) => {
  const { default: mexp } = await import(
    'https://cdn.skypack.dev/math-expression-evaluator'
  )
...
test.skip('subscript-refs', async (t) => {
  const {
    ...
    parse: dparse,
    evaluate,
  } = await import('./lib/parser/subscript-refs.js')
  console.time('dislex')
  for (let i = 0; i < RUNS; i++) {
...
test.skip('subscript-v1', async (t) => {
  const {
    ...
    parse: v1parse,
    evaluate,
  } = await import('./lib/parser/subscript-lex.js')
...
test.skip('direct fn', async (t) => {
  const createFn = (n) => (a, b, c, d, f, i, k) =>
    1 + (((a * b) / c) % d) - 2.0 + (-3e-3 * +4.4e4) / f.g[0] - i.j(+k == 1)(n)
