...
export let idx,
  ...
  err = (
    ...
    const after = cur
      ...
      .shift()
    throw EvalError(
      `${msg} at ${lines.length}:${last.length} \`${idx >= 108 ? '…' : ''}${before}┃${after}\``,
      'font-weight: bold',
    )
  },
  ...
  binary = (op, prec, right = false) =>
    token(
      op,
      prec,
      (a, b) => a && (b = expr(prec - (right ? 0.5 : 0))) && [op, a, b],
    ),
  // post indicates postfix rather than prefix operator
  unary = (op, prec, post) =>
    token(op, prec, (a) =>
      post ? a && [op, a] : !a && (a = expr(prec - 0.5)) && [op, a],
    ),
  ...
  group = (op, prec) =>
    token(op[0], prec, (a) => !a && [op, expr(0, op.charCodeAt(1))]),
  // register a(b), a[b], a<b> etc,
  // NOTE: we make sure `null` indicates placeholder
  access = (op, prec) =>
    token(op[0], prec, (a) => a && [op, a, expr(0, op.charCodeAt(1)) || null])
