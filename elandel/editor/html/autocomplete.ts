...
const defaultTagCompletions: TagCompletion[] = [
  ['a', '<a href="${1}">${2}</a>'],
  ['article', '<article${1}>\n  ${2}\n</article>'],
  ['aside', '<aside${1}>\n  ${2}\n</aside>'],
...
/**
 * Create completion functions for tags, attribute name/value
 */
function createTemplateTagCompletionSource(props: any): CompletionSource {

  // Extend autocomplete with language definition
  ...
  /**
   * When user types the first letter of tag
   */
  const completionsWithBracket: Completion[] = []

  function addTagCompletion(label: string, snippet: string) {
    ...
    completionsWithBracket.push(
      snippetCompletion(snippet, { label })
    )
  }
  ...
  function completeTagAttributeValue(
    state: EditorState,
    tree: SyntaxNode,
    from: number,
    to: number,
    ...
    if (nameNode) {
      ...
      if (!attrs) {
        let elt = findParentElement(tree),
          info = elt ? Tags[elementName(state.doc, elt)] : null
  ...
  /**
   * Extend with language definition
   */
  const {
    languageDefinition = {
      tags: {}
  ...
  if (htmlTags) {
    for (const [label, snippet] of defaultTagCompletions) {
      addTagCompletion(label, snippet)
    }  
  }
  ...
  for (const name of Object.keys(languageDefinition.tags)) {
    ...
    const snippet = config.snippet ||
      (config.closed ? `<${name}\${1} />` : `<${name}\${1}>\${2}</${name}>`)

    addTagCompletion(name, snippet)
  }

  function templateTagCompletionSource(
    ...
    if (
      ...
      return completeTagAttributeName(
        state,
        tree,
    ...
    if (
      ...
      return completeTagAttributeValue(
        state,
        tree,
  ...
  return templateTagCompletionSource
}

export function createHtmlAutocomplete(props: any = {}) {
  return autocompletion({
    ...
    override: [
      createTemplateTagCompletionSource(props)
    ],
  })
}
