...
const defaultPackageName = (savedState && savedState.name)
  || 'tangible-templates'
...
const Exporter = () => {
  ...
  const ensureTemplateTypeItemOptions = async (templateType) => {
    ...
    log('ensureTemplateTypeItemOptions', templateType, templateTypeItemOptionsRef.current)

    const promise = ajax(
      ajaxActionPrefix+'get_template_type_item_options',
      {
  ...
  return <div id="exporter">

    <div className='export-name'>
      Package name: <input type="text" name="name"
    ...
    <div className='export-rules'>
      ...
      { exportRules.map((rule, ruleIndex) =>
        <ExportRule key={`export-rule-${ruleIndex}`} {...{
          rule,
          ruleIndex,
          ...
          onClick={() => {
            ...
            ajax(
              ...
              .then(result => {
                ...
                const timestamp = new Date().toISOString().slice(0, 10).replace(/-/g, '') // Ymd
