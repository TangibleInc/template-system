 ...
 */
const {
  isTangibleBlocksProInstalled = false
} = window.Tangible
...
const Catalog = ({ tabs, activeTab, loadImportData }) => {
  ...
  const attemptInstall = (dld) => {
    ...
    if( dld.dependencies && dld.dependencies.unmet.length > 0 ) {
      if( !confirm('Warning: There are one or more missing plugins which this block distribution relies on. If you proceed with the installation, the installed templates might interfere with the normal functioning of your website.') ) {
        return
      }
    }
  ...
  return(<div className="tangible-blocks__loop-gallery">
      ...
      <div className="theme-browser">
        <div className="themes wp-clearfix">
          ...
          {catalogState.downloads.map((dld, idx) =>
            <div key={`download-${idx}`} className={'theme' + (dld.is_pro? ' pro': '') }>

              <div className={'theme-screenshot' + (!dld.thumbnail? ' blank': '') }>

                {dld.thumbnail &&
                  <img src={dld.thumbnail}/>
                }

                {dld.vendor &&
                <div className="theme-vendor-wrap">
                  {dld.vendor.logo &&
                  <img className="theme-vendor-logo" src={dld.vendor.logo} />
                  }
                  <a className="theme-vendor-link" target="_blank" href={dld.vendor.url}>{dld.vendor.name}</a>
                </div>
                }
                ...
                <div className="theme-copy">
                  ...
                  <p className="theme-details">
                    ...
                    {dld.dependencies.met.map((dep, idx) =>
                      <span key={`dependency-met-${idx}`}
                        className="dependency met" data-slug={dep.slug}>{dep.name}</span>
                    )}
                    {dld.dependencies.unmet.map((dep, idx) =>
                      <span key={`dependency-unmet-${idx}`}
                        className="dependency unmet" data-slug={dep.slug}>{dep.name}</span>
                    )}
                  </p>
                  }
                </div>
                ...
                <div className="theme-actions">
                  { ['public', 'free'].includes(activeTab) ?
                    <button className="button button-primary"
                      ...
                      onClick={() => attemptInstall(dld)}>Install</button> :
                    <a href={dld.link} target="_blank" className="button">Buy</a>
                  }
                </div>
              </div>
