  ...
  const partSelect = partName => {
    ...
    if (!isPartSelect) {
      ...
      if (type==='input') {
        ...
        return <div className={`rule-part rule-${partName}`}>
    ...
    const options = isOperators
      ? fieldDef[ partName ]
        ...
        .filter(obj =>
          ! obj.field_2 || obj.field_2.indexOf( rule.field_2 ) >= 0
        )
    ...
    if (
      ...
      && !value && !labelForEmptyValue && options[0] && options[0].value
    ) {
  ...
  return <>
    <div className='rule'>
      <div className="rule-parts">
        ...
        { fieldDef && (!fieldDef['field_2'] || rule['field_2']) && partSelect('operators') }
        { fieldDef && (!fieldDef['field_2'] || rule['field_2']) && partSelect('values') }
      </div>
