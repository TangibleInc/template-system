...
const Importer = ({
  ...
  const onError = (message) => setInputState({
    ...inputStateRef.current,
    importing: false,
    importData: {},
    importedData: {},
  ...
  const importJSON = (data) => {
    ...
    ajax(
      ...
      .then(result => {
        ...
        const importedData = {
          post_types: Object.keys(importData.post_types)
            .reduce((obj, postType) => {
              ...
              obj[postType] = importData.post_types[postType]
                .filter(post => duplicateIds.indexOf(post.id) < 0)
        ...
        setInputState({
          ...
          importData: result.duplicates_found.length
            ? importData
            : {} // Clear
          ,
          importedData,
          message: result.post_count + ' template'
          + (result.post_count !== 1 ? 's' : '')
          + ' imported'
          + (result.failed_post_count
            ? ' - ' + result.failed_post_count + ' template'
  ...
  return <div id="importer">
...

    {
      ...
      Object.keys(inputState.importedData.post_types || {}).map(postType =>
        inputState.importedData.post_types[ postType ]
        && inputState.importedData.post_types[ postType ].length > 0
        && <>
          <div><b>{ postTypeToLabel[postType] || 'Templates' }</b></div>
          <ul>
            { inputState.importedData.post_types[ postType ].map((post, postIndex) =>
      ...
      <>
        <p style={{ fontWeight: 'bold', color: 'red' }}>
          { inputState.duplicatesFound.length } duplicate template{
            inputState.duplicatesFound!==1 ? 's' : ''
          } found
        </p>

        {
          ...
          Object.keys(postTypeToLabel).map(postType => {
            const posts = inputState.duplicatesFound.filter(post => post.post_type===postType)
