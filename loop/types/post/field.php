...
$loop->get_post_field = function( $post, $field_name, $args = [] ) use ($loop) {
  ...
  switch ( $field_name ) {
    ...
    case 'publish_date': $value = $post->post_date; break;
    case 'modify_date':  $value = $post->post_modified; break;

    case 'url': $value = get_permalink($id); break;
    case 'edit_url': $value = get_edit_post_link($id, false); break;

    // @see https://developer.wordpress.org/reference/functions/post_class/
    case 'post_class': $value = join( ' ', get_post_class('', $id) ); break;

    case 'menu_order': $value = $post->menu_order; break;
